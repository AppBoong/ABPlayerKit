...
- An explicit `pause()` issued while the app is backgrounded is no longer overridden on foreground return. `willResignActive` captures whether playback was live, and `willEnterForeground`'s resume is gated on that capture alone; `ABPlayer.pause()` cleared the playback intent but left the capture set, so returning to the foreground restarted playback the user had deliberately stopped. Only `ABBackgroundPolicy.continueAudioOnly` could reach this — it is the only policy that leaves playback running in the background, and therefore the only one where a user can pause it there (the lock screen, `ABPlayerKitNowPlaying`, or the controls overlay); under every other capturing policy the library itself pauses before backgrounding, so nothing could contradict the capture. `pause()` now retires the capture, and an explicit pause stays paused. The background policy machine's own pause action and the audio-interruption/route-change handlers reach the playback target directly rather than through `pause()`, so their resume-on-foreground behavior is unchanged.
...
- A view whose layout settles one pixel differently between passes can no longer drive an endless tuning re-apply loop. `ABPlayerView` reports its pixel size on every layout pass, and the size it reported alternated between two neighbouring values; each alternation re-resolved `ABPlaybackTuning.displaySizeSentinel` to a cap one pixel different, re-applied the tuning, and broadcast `.tuningApplied`. Any consumer that re-renders on that event fed the broadcast straight back into the next layout pass, so the two drove each other at display refresh rate indefinitely — measured on device at ~92 re-applies per second, with `AVPlayer`'s own `timeControlStatus` KVO re-firing in lockstep and the UI effectively unusable. A reported size now has to move by at least 16 pixels in one dimension before it counts as a different size (a decode-resolution cap is only meaningful at macroblock granularity), measured against the last stored size so that jitter cannot accumulate into a drift, and the tuning is re-applied only when the value that actually reaches `AVPlayerItem` changes. **Behavior change**: the applied `preferredMaximumResolution` may now trail a view's true pixel size by up to 16 pixels, and a tuning carrying no `displaySizeSentinel` is no longer re-applied on a resize at all (it resolves to the same value at every size). Genuine resizes — rotation, split view, a feed cell changing size — still re-cap, once the change clears that 16-pixel tolerance; a resize smaller than it now leaves the cap where it was.
