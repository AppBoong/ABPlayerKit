...
``ABPlayerControlsStyle/skipBackwardIcon``/``ABPlayerControlsStyle/skipForwardIcon`` are `ABControlIcon?`, so a bare `.none` there resolves to `Optional.none` (`nil`) — the sentinel below that *derives* an icon from the skip interval, not the case that hides the button. Spell out the type, `ABControlIcon.none`, to reach the hiding case. `playIcon`/`pauseIcon` aren't optional, so plain `.none` on those would be a compile error, not a silent no-op — the ambiguity is specific to the two optional skip icons.
