...
설계 → 리뷰·audit → 구현을 분리된 단계로 운영한다. 각 단계는 앞 단계의 **확정된 산출물만** 입력으로 받고, 트레이드오프가 걸린 쟁점은 단계 안에서 임의로 정하지 않고 사용자 결정으로 올린다.

> 2026-08-03 사용자 확정.

## 6. 설계 결정 방식 및 기본 스탠스 (Phase 1 출발점)

**결정 프로세스**: 설계 초안 → 트레이드오프 쟁점은 사용자 결정 → 설계 문서 audit 승인 → 구현.
