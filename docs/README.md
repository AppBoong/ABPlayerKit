...
Everything else under `docs/` — `DESIGN-*.md`, `BRIEF-*.md`, `PLANNING.md`, `IMPL-*.md`, `CHECKLIST-*.md`, and `briefs/` — is a maintainer-facing design and implementation record: open questions and the decisions made on them, work-package briefs handed to an implementing agent, and the results reported back. It documents *how* and *why* the library reached its current shape, for whoever maintains it next.
