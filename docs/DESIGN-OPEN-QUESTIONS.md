...
| Q6 | 오버레이 주입 방식 | **A** — **UIKit `UIView` 오버레이만 지원.** SwiftUI 소비자는 직접 `UIHostingController` 래핑. 원본이 겪은 호스팅 어긋남 위험을 라이브러리가 떠안지 않음 | 추천(C)과 다른 사용자 결정. **2026-08-05 Q6-A로 개정, 아래 참조** |
| Q6-A | (개정) `ABPlayerControlsView.accessoryViews`/SwiftUI 오버레이 주입 방식 재검토 | **B 계열** — Q6의 "UIKit 전용" 결정을 뒤집고, `ABPlayerControls`/`ABVideoPlayerWithControls`에 `@ViewBuilder accessories:` 이니셜라이저를 **additive**로 추가한다(기존 `accessoryViews: [UIView]` 이니셜라이저는 deprecate하되 유지, `ABPlayerControlsView.accessoryViews` UIKit API는 그대로 유지·deprecate 금지). 내부적으로 `ABAccessoryHostingBox`가 `UIHostingController`를 소유하고 `sizingOptions`/투명 배경/부모 VC 부착(responder chain 탐색)을 명시적으로 처리한다 | **재결정 사유**: SwiftUI 라이브러리로서의 API 완성도 — "UIKit 오버레이만 지원"은 2026년 시점 SwiftUI 중심 소비자에게 채택률 손해가 Q6이 우려한 호스팅 어긋남 위험보다 크다고 판단. **완화책**: `ROADMAP-round4.md` WP-B1의 부모 VC 부착 전략(`didMoveToWindow` 시점 responder chain으로 가장 가까운 `UIViewController` 탐색 → `addChild`/`didMove(toParent:)`, 부모를 못 찾는 경우는 DocC·코드 주석에 명시해 조용히 넘어가지 않음) — Q6이 지목한 위험을 라이브러리가 명시적으로 흡수한다. **결정일**: 2026-08-05. 사용자 승인 완료, `ROADMAP-round4.md` (B) 트랙(WP-B0~B5)으로 구현 |
