...
Three defects that shipped-quality automated verification did not catch, what the
tests were measuring instead, and what changed as a result.

At the point these were found, ABPlayerKit had **743 tests at 91% line coverage**
...
green. Two of the three defects below were found by a person holding an iPhone;
the third was only reachable because the first one had been fixed.

The interesting part is not that bugs existed. It is that in each case the suite
contained a test *specifically aimed at the defect*, which passed — because it
measured the wrong dimension, the wrong object, or the wrong quantity.
...
For a library that wraps `AVPlayer`, a large class of behavior is defined by what
iOS does in response to what you did, and *when* you did it. The iOS Simulator has
no Picture in Picture, no lock screen, no AirPlay receiver, and no background audio
...
carried the entry that later paid out, written while the feature was still being
designed:

> | 10-9 | `.continueAudioOnly`가 레이어 detach만으로 실제 배경 오디오를 이어 가는가 | **플랫폼 기법에 근거한 설계 판단, 리포 코드로는 검증 불가** | §6.2의 3조건을 갖춘 데모에서 **기기 수동 확인** |

*("Does `.continueAudioOnly` actually sustain background audio through layer detach
alone? — A design judgment based on a platform technique; **not verifiable from repo
code**. Mitigation: **manual device verification** in a demo satisfying the three
preconditions.")*
...
Its opening paragraph states the claim it is defending:

> CI가 전부 그린인 것과 기기에서 기능이 살아 있는 것은 **별개의 주장**이다.
>
> *(CI being all green and the feature being alive on the device are **separate claims**.)*
...
and an attached video layer in a backgrounded app means AVFoundation stops the
player first.

The lifecycle observer registered a block-form notification handler that did this:
...
Everything downstream of `onBackground()` is synchronous — the policy machine
returns `[.setLayerAttachment(false)]`, the engine flips its flag and broadcasts,
and `ABPlayerView` responds by setting `playerLayer.player = nil`. Four synchronous
hops, all of them displaced by the single `Task {}` at the top. `post(name:)`
returned with nothing done; AVFoundation stopped the still-attached player; nothing
was playing when iOS evaluated the assertion; the process was suspended a few
seconds later.

`willResignActive` had the same requirement for an unrelated reason: its only job is
`wasPlayingBeforeBackground = target.isPlaying`, and a turn later decode teardown has
already falsified that value.
...
Post, yield, assert the end state. **The yields exist precisely to absorb the hop the
defect lived in.** All five tests in that file had the same shape, and all five pass
against the broken code — confirmed by reverting the observer and running them.

This was not an oversight anyone had failed to notice. A sibling suite documented the
hop *explicitly*, as an inert implementation detail to be worked around:
...
surfaced, and treats it as a scheduling nuisance. It carries a review ID, so a
reviewer signed off on it.

**There was a second, independent gap.** Every pre-fix assertion stopped at
`player.isLayerAttachmentEnabled` — the engine's *flag*. What iOS actually reads is
`AVPlayerLayer.player`, set two synchronous hops further downstream in the view. No
test in the repository bound an `ABPlayerView` and asserted on its layer across a
background transition. Even a correct engine could have been undone by a later change
to the view, with a fully green suite.

So the accurate diagnosis is: **the tests measured the wrong dimension (state, not
timing) on the wrong object (the engine's proxy, not the thing the platform reads).**
...
`post(name:)` returned?"* — which is the same question iOS asks.

A companion test binds a real `ABPlayerView` and asserts `layer?.player == nil`,
closing the second gap. Both fail against the reverted observer; the five original
...
racing silently. The assumption does not disappear; it moves somewhere it is
...
**Commit [`d484ea3`](https://github.com/AppBoong/ABPlayerKit/commit/d484ea3) · found in review, on a path device verification had just resurrected · 747 tests green**

Provenance first, since it matters: this one was **not** found on the device. It was
a review finding, and the device checklist recorded the relevant item as a clean
pass. What makes it belong here is that it was *unreachable* until the previous fix
landed — `.continueAudioOnly` is the only policy that leaves playback running in the
...
Under every other policy the library itself pauses before backgrounding, so nothing
could contradict the capture and the flag was effectively unfalsifiable. Under
`.continueAudioOnly`, a user pausing from the lock screen went through the public
`ABPlayer.pause()`, which cleared `desiresPlayback` but left the capture set — and
`willEnterForeground` then overrode the user's explicit intent.

The fix is one line, plus ten of comment explaining which callers must *not* be
affected:
...
The state machine was covered exhaustively — parameterized over grade, prior state,
...
func willEnterForegroundResumesConditionally(grade: ABPlaybackGrade, wasPlaying: Bool) {
    for policy in [ABBackgroundPolicy.pause, .pauseAndDetachLayer, .continueAudioOnly] {
        let actions = machine.actions(
            ...
            policy: policy,
            grade: grade,
            wasPlayingBeforeBackground: wasPlaying,   // ← injected
...
**The lesson: the suite tested the state machine exhaustively and the state machine's
inputs not at all.** Exhaustive coverage of a pure function proves the function; it
says nothing about the impure code that feeds it.

The regression test added with the fix ships alongside a *control* case asserting that
`.pause` policy's own background-pause-then-resume is unchanged — the fix touches the
public `pause()`, and the policy machine reaches `target.pause()` directly, so the
control is what proves the blast radius is what the comment claims.
...
`ABPlayerView` reports its pixel size to the engine on every layout pass so the
engine can cap decode resolution to what is actually on screen. On device, the
reported size alternated between two values one pixel apart — `1164×655` and
`1165×655`. Legal host behavior, and nothing a library can prevent.
...
20 seconds on an iPhone Air** (≈92/s), with `AVPlayer`'s `timeControlStatus` KVO
re-firing in lockstep, because every apply writes
`automaticallyWaitsToMinimizeStalling`. The UI was unusable.

Note what closes this loop: it runs *through the consumer*. Nothing inside the
library observes it, which is why no amount of unit testing the library in isolation
had a chance.
...
func repeatedIdenticalDisplaySizeDoesNotReapply() {
    ...
    player.reportDisplaySize(cellSize)   // the *identical* size

    #expect(applyTuningCallCountAfterSecond == applyTuningCallCountAfterFirst)
}
```

It reports the **identical** size twice. It verifies the `!=` guard's stated contract
...
Reproduced as a test, the pre-fix code amplifies at 100%: 400 alternating reports
produce **402 applies and 400 broadcasts** against expectations of 2 and 0. Four
guard-rail tests accompany it — rotation still re-caps, a view growing one pixel at a
time re-caps exactly once when it clears the tolerance, `.zero` transitions always
count — and two of those pass against *both* versions, which is what makes them guard
rails rather than restatements of the fix.
...
**1. When timing is the contract, assert the timing.** A test that yields before
asserting measures the end state and nothing else. Where synchrony is load-bearing,
the test body is deliberately non-`async`, so the runtime cannot hand the actor over
and the assertion becomes *"this already happened."*

**2. Assert on the object the platform reads, not the proxy your code updates.**
`isLayerAttachmentEnabled` is the library's opinion. `AVPlayerLayer.player` is the
fact. Pinning only the first left the second free to regress silently.

**3. A pure-reducer test cannot check who maintains its inputs.** Parameterizing a
reducer over its whole input space proves the reducer. The defect will be in the
impure code that decides what to pass it — which needs its own tests, at the level
...
reported as failing three separate times for three procedural reasons — a tester on
the wrong tab (which lacks the controls the item requires), leftover audio from the
previous item contaminating the pass criterion, and the Ring/Silent switch — before
the real defect was isolated. Each produced a hardening of the checklist: explicit tab
names, an explicit precondition to stop the prior item's session, and a demo that
takes a `.playback` audio session at startup. **Distinguishing "the feature is broken"
from "the procedure is broken" cost more time than fixing the defect did.** A manual
checklist with ambiguous steps generates false failures at roughly the rate a flaky
test does, and is debugged the same way.

---

## The numbers

Test counts are `@Test` declarations across the five suites, which on this codebase
equals the executed case count (no parameterized test expands). Verified against the
CI logs for the corresponding runs.
...
| `d484ea3` | **747** | Explicit-pause resurrection fixed, + control case. |
| `14670be^` | 747 | Tuning loop live on device at ~92 re-applies/s. Suite green. |
| `14670be` | **752** | Fixed. Five new tests, two of them guard rails. |
| v0.4.0 shipped | **752** | 91.1% line coverage. |

CI at every one of those points: `build-and-test` (Xcode 16.4, warnings as errors,
DocC warnings as errors), `lint` (`swiftlint --strict`), `thread-sanitizer`, and a
coverage job. Four jobs, all green, throughout.
